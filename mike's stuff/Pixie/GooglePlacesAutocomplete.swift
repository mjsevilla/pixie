...
public class Place: NSObject {
   ...
   public convenience init(prediction: [String: AnyObject], apiKey: String?) {
      ...
      desc = desc.stringByReplacingOccurrencesOfString(", United States", withString: "")
      self.apiKey = apiKey
   }
...
public class PlaceDetails: Printable {
   ...
   public init(json: [String: AnyObject]) {
      ...
      let addressInfo = result["address_components"] as! [AnyObject]
      let geometry = result["geometry"] as! [String: AnyObject]
      let location = geometry["location"] as! [String: AnyObject]
      
      for info in addressInfo {
         for type in info["types"] as! [String] {
            if type == "locality" {
               self.city = info["long_name"] as! String
            }
            else if type == "administrative_area_level_1" {
               self.state = info["short_name"] as! String
            }
         }
      }
   ...
   public var description: String {
      return "PlaceDetails: \(city), \(state) (\(latitude), \(longitude))"
   }
}
